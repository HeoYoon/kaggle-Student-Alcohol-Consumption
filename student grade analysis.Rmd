...
d_index <- createDataPartition(rf_data$setting, p = 0.6, list = F)
train_data <- rf_data[d_index,]
test_data <- rf_data[-d_index,]

rf.1 <- randomForest(x = train_data[,-12],  y = train_data[,12],importtance = TRUE, ntree = 1000)
rf.1
varImpPlot(rf.1)
...
train_data <- train_data %>%
  select(-address, -studytime)

test_data <- test_data %>%
  select(-address, -studytime)

rf.2 <- randomForest(x = train_data[,-10],  y = train_data[,10],importtance = TRUE, ntree = 1000)
rf.2
varImpPlot(rf.2)

cv10_1 <- createMultiFolds(train_data[,10], k = 10, times = 10)

ctrl_1 <- trainControl(method = "repeatedcv", number = 10, repeats = 10, index = cv10_1)

set.seed(3231)
rf.5 <- train(x = train_data[,-10], y = train_data[,10], method = "rf", tuneLength = 3, ntree = 1000, trControl = ctrl_1)

rf.5

pr.rf = predict(rf.5, newdata = test_data)

confusionMatrix(pr.rf, test_data$setting)
```
